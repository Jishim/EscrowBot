...
// ── Remember every user who ever messages the bot, so we can notify sellers later ──
bot.use(async (ctx, next) => {
  if (ctx.from && ctx.from.username) {
    supabase
      .from('users')
      .upsert({ telegram_id: String(ctx.from.id), username: ctx.from.username.toLowerCase() })
      .then(({ error }) => {
...
// Escapes text so user input (which may contain _ * < > etc.) can't break Telegram's formatting
...
// ── Admin-only: /wipeuser username — clears a user's deal history and known-user record ──
// Only works when typed inside your private admin group, to keep it locked down.
bot.command('wipeuser', async (ctx) => {
  if (!ADMIN_CHAT_ID || String(ctx.chat.id) !== String(ADMIN_CHAT_ID)) {
    return; // silently ignore if not sent from the admin group
  }

  const parts = ctx.message.text.trim().split(/\s+/);
  const target = parts[1] ? parts[1].replace('@', '').toLowerCase() : null;

  if (!target) {
    return ctx.reply('Usage: /wipeuser username (without @)');
  }
  ...
  let deletedDeals = 0;
  if (userRow) {
    const { data: byBuyer } = await supabase.from('deals').delete().eq('buyer_id', userRow.telegram_id).select();
    deletedDeals += byBuyer ? byBuyer.length : 0;
  }
  const { data: bySeller } = await supabase.from('deals').delete().eq('seller_username', target).select();
  deletedDeals += bySeller ? bySeller.length : 0;

  await supabase.from('users').delete().eq('username', target);

  await ctx.reply(`🧹 Wiped @${target}: removed ${deletedDeals} deal(s) and their known-user record.`);
});
...
bot.action(['pay_BTC', 'pay_USDT(ERC20)', 'pay_ETH'], async (ctx) => {
  ...
  // Safety check: if the session lost its data (e.g. bot restarted mid-flow),
  // don't create a broken deal — ask the person to start over cleanly.
  if (!session.data.buyer_id || !session.data.amount || !session.data.currency || !session.data.seller_username) {
    await ctx.answerCbQuery();
    return ctx.reply('⚠️ Your session expired or was interrupted. Please tap 🤝 Start Deal to begin again.');
  }
  ...
  await ctx.reply(
    `🔖 Transaction ID: ${dealId}\n💳 Payment Method: <b>${escapeHtml(method)}</b>\n💰 Amount: <b>${session.data.amount.toFixed(1)} ${escapeHtml(session.data.currency)}</b>\n📊 Escrow Fee: $${session.data.fee}\n📝 Description: ${escapeHtml(session.data.description)}\n👤 Seller: @${escapeHtml(session.data.seller_username)}\n\n⚠️ Double check the network before sending funds.\n\nAfter payment:\n1. Send the funds\n2. Tap "I Have Paid"\n3. Upload your payment screenshot`,
    {
      parse_mode: 'HTML',
      ...Markup.inlineKeyboard([
  ...
  // Try to notify the seller directly, if they've messaged the bot before
  const { data: sellerUser } = await supabase
    .from('users')
    .select('telegram_id')
    .eq('username', session.data.seller_username.toLowerCase())
    .maybeSingle();
  ...
  if (sellerUser) {
    try {
      await ctx.telegram.sendMessage(
        ...
        `🤝 New Escrow Deal — you've been named as the seller!\n\n🔖 Transaction ID: ${dealId}\n💰 Amount: ${session.data.amount.toFixed(1)} ${session.data.currency}\n📝 Description: ${session.data.description}\n\nThe buyer will send payment to escrow shortly. You'll be notified once it's confirmed.`
      );
    } catch (err) {
      console.error('Failed to notify seller:', err.message);
    }
  } else {
    await ctx.reply(`ℹ️ @${session.data.seller_username} hasn't started this bot yet, so they weren't notified automatically. Ask them to send /start to the bot so they can be reached for future deals.`);
  }
  ...
  if (session.step === 'awaiting_payment_proof' && dealId) {
    ...
    if (ADMIN_CHAT_ID) {
      ...
      try {
        ...
        console.error('Failed to notify admin group:', err.message);
      }
    }
    session.step = null;
    return;
...
bot.launch().catch((err) => {
  console.error('Bot failed to launch (often means another instance is already running with this token):', err.message);
});
