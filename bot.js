...
bot.on('photo', async (ctx) => {
  ...
  if (session.step === 'awaiting_payment_proof' && dealId) {
    await supabase.from('deals').update({ status: 'proof_submitted', payment_proof_url: photoId }).eq('id', dealId);
    await ctx.reply('Wait for the seller and the admin to approve your payment‼️');
